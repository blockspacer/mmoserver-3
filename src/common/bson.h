...
static void init_oid_header() {
	...
	oid_counter =static_cast<uint32_t>( h ^ time(NULL) ^ (uintptr_t)&h);
}
...
int lobjectid(lua_State *L) {
	...
	if (lua_isstring(L, 1)) {
		...
		int i;
		for (i = 0; i < 12; i++) {
			oid[i] = hextoint(str[i * 2]) << 4 | hextoint(str[i * 2 + 1]);
		}
	}
	else {
		time_t ti = time(NULL);
		...
		oid[2] = (ti >> 8) & 0xff;
		oid[3] = ti & 0xff;
		memcpy(oid + 4, oid_header, 5);
		oid[9] = (oid_counter >> 16) & 0xff;
		oid[10] = (oid_counter >> 8) & 0xff;
		oid[11] = oid_counter & 0xff;
		++oid_counter;
	}
	char tmp[24];
	toHexLower(oid, 12, tmp);
	lua_pushlstring(L, (const char*)tmp, 24);
...
static void pack_ordered_dict(lua_State *L, struct bson *b, int n, int depth);
...
ldecode(lua_State *L) {
	...
	if (data == NULL) {
		...
		struct bson_reader br = { b , str_len };
