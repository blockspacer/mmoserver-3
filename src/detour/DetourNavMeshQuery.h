...
public:
	...
	dtStatus rayCastOnPoly(dtPolyRef, const float *start, const float *end, float &t, float &nearest_dist_sqr, float *nearest_pt) const;

	// gets the height of any (x, z) pos in mesh within radius, looks like capsule intersection
	dtStatus GetHeight(float x, float z, float radius, const dtQueryFilter* filter, float *height) const;
