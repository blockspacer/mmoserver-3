...
static bool RaycastTriangle(const float *p0, const float *p1,
	const float *vert0, const float *vert1, const float *vert2,
	float &min_t/*in and out*/,
	float &nearest_dist_sqr/*in and out*/,
	float *nearest_point)
{
	float v10[3];
	dtVsub(v10, vert1, vert0);
	float v20[3];
	dtVsub(v20, vert2, vert0);

	// cross
	float triangleNormal[3];
	dtVcross(triangleNormal, v10, v20);

	const float dist = dtVdot(vert0, triangleNormal);
	float dist_a = dtVdot(triangleNormal, p0);
	dist_a -= dist;
	float dist_b = dtVdot(triangleNormal, p1);
	dist_b -= dist;
	...
	float point[3];
	dtVsub(point, p1, p0);
	dtVscale(point, point, distance);
	dtVadd(point, p0, point);

	float nearest[3];
	if (dtClosestPtPointTriangle(nearest, point, vert0, vert1, vert2))
	{
		min_t = distance;
		return true;
	}

	// already hit other triangle, not need to check nearest
	if (min_t < 1.0f)
	{
		return false;
	}

	dtVsub(point, nearest, point);
	float dist_sqr = dtVdot(point, point);
	if (dist_sqr < nearest_dist_sqr)
	{
		nearest_dist_sqr = dist_sqr;
		dtVcopy(nearest_point, nearest);
	}
	return false;
}

dtStatus dtNavMeshQuery::rayCastOnPoly(dtPolyRef ref, const float *start, const float *end, float &prev_min_t, float &nearest_dist_sqr, float *nearest_pt) const
{
	dtAssert(m_nav);
	const dtMeshTile* tile = 0;
	const dtPoly* poly = 0;
	...
	for (int j = 0; j < pd->triCount; ++j)
	{
		...
		if (RaycastTriangle(start, end, v[0], v[1], v[2], min_t, nearest_dist_sqr, nearest_pt))
		{
			hit = true;
		}
	}
...
dtStatus dtNavMeshQuery::GetHeight(float x, float z, float radius, const dtQueryFilter* filter, float *height) const
{
	dtAssert(m_nav);
	...
	dtPolyRef polys[128];
	int polyCount = 0;
	float center[3] = { x, .0f, z };

	float extents[3] = { .01f, 4000.f, .01f }; // small range
	...
	dtPolyRef nearest = 0;
	float nearest_dist_sqr = FLT_MAX;
	float min_t(1.0f);
	bool hit = false;
	float start[3] = { x, 5000.0f, z };
	float end[3] = { x, -5000.0f, z };
	float nearest_pt[3];

	for (int i = 0; i < polyCount; ++i)
	{
		float dist_sqr(nearest_dist_sqr);
		if (DT_SUCCESS == rayCastOnPoly(polys[i], start, end, min_t, dist_sqr, nearest_pt))
		{
			nearest = polys[i];
			hit = true;
			continue;
	...
	if (hit)
	{
		*height = start[1] + (end[1] - start[1]) * min_t;
		return DT_SUCCESS;
	}
	else if (nearest_dist_sqr < radius * radius)
	{
		*height = nearest_pt[1];
		return DT_SUCCESS;
	}
